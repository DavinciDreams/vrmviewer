...
export interface ModelRecord {
  ...
  license?: string;
  thumbnail?: string;
  /** Transient data URL used to persist captured thumbnails to the server library. */
  thumbnailDataUrl?: string;
  /** Server timestamp used to bust cached model card thumbnails. */
  ...
  // --- Asset-library grouping fields ---

  /** Stable family id for variants that represent the same source asset. */
  assetGroupId?: string;
  /** Level-of-detail tier for generated asset variants. */
  lodTier?: 'default' | 'lod0' | 'lod1' | 'lod2' | 'lod3';
  /** Polycount from the upstream manifest when available. */
  polycount?: number | null;
  /** Texture size from the upstream manifest when available. */
  textureSize?: number | null;
  /** Upstream manifest path or provider-specific source URI. */
  sourcePath?: string;
  /** Source pack slug, e.g. maple_japanese_v1. */
  packSlug?: string;
  /** Human-readable source pack label. */
  packName?: string;
  /** Coarse asset quality gate for creator/runtime workflows. */
  qualityTier?: 'draft' | 'game_ready' | 'hero';
  /** Review state used to keep rough assets visible but out of curated runtime pools. */
  reviewStatus?: 'needs_regen' | 'pending' | 'approved' | 'rejected';
  /** Latest game/export pipeline output paths and manifests keyed by target. */
  exports?: {
    game?: ModelExportRecord;
    store?: ModelExportRecord;
  };
  /** Marketplace/product copy generated by Hill/Nemotron, keyed by platform. */
  marketplaceCopy?: Record<string, unknown>;
  /** Suggested product price populated by Hill's marketplace pipeline. */
  priceUsd?: number;
  /** Latest export job id associated with this asset. */
  latestExportJobId?: string;
}

export interface ModelExportRecord {
  jobId: string;
  destination: 'game' | 'store' | 'library';
